...
const FIM={ql:"Quads",qr:"Quads",cfl:"Calves",cfr:"Calves",ad:"Adductors",bil:"Biceps",bir:"Biceps",pcl:"Chest",pcr:"Chest",au:"Core",am:"Core",al:"Core",ol:"Core",or:"Core",dl:"Front Delts",dr:"Front Delts",fal:"Forearms",far:"Forearms"};
const BIM={gl:"Glutes",gr:"Glutes",hl:"Hamstrings",hr:"Hamstrings",cbl:"Calves",cbr:"Calves",trl:"Triceps",trr:"Triceps",trapl:"Traps",trapr:"Traps",latl:"Lats",latr:"Lats",rhb:"Upper Back",erl:"Lower Back",err:"Lower Back",rdl:"Rear Delts",rdr:"Rear Delts",fbl:"Forearms",fbr:"Forearms"};
...
function getIds(muscles=[]){const f=new Set(),b=new Set();muscles.forEach(m=>{const x=MM[m];if(x){x.f.forEach(i=>f.add(i));x.b.forEach(i=>b.add(i))}});return{f:[...f],b:[...b]};}
...
// anyActive: true when user has tapped an exercise
function MuscleDefs({ids,map,pri,sec,dim,anyActive}){
  return(
    <defs>
      {ids.map(id=>{
...
function gF(id){return `url(#mg_${id})`;}
function gS(id,map,pri,sec,dim,anyActive){
  const base=MC[map[id]]||"#888888";
...
function gW(id,pri,sec,anyActive){
...
function FrontBody({pri=[],sec=[],dim=[],anyActive=false}){
  const p=(id)=>({fill:gF(id),stroke:gS(id,FIM,pri,sec,dim,anyActive),strokeWidth:gW(id,pri,sec,anyActive),strokeLinejoin:"round"});
  return(
...
function BackBody({pri=[],sec=[],dim=[],anyActive=false}){
  const p=(id)=>({fill:gF(id),stroke:gS(id,BIM,pri,sec,dim,anyActive),strokeWidth:gW(id,pri,sec,anyActive),strokeLinejoin:"round"});
  return(
    <svg viewBox="0 0 120 260" style={{height:190,width:"auto",display:"block"}}>
...
function AnimFigure({type,color}){
  const c=color||"#999";
  const s={stroke:c,strokeWidth:"2",strokeLinecap:"round",fill:"none"};
  const sf={fill:c,stroke:"none"};
  switch(type){
...
export default function App(){
  ...
  const handleDay=i=>{setActiveDay(i);setActiveEx(null);};
